...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Adiciona evento de clique com botão direito nas músicas
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const updateRangeProgress = (rangeElement) => {
        const percent = ((rangeElement.value - rangeElement.min) / (rangeElement.max - rangeElement.min)) * 100;
...
async function getStreamUrl(trackUrl) {
    try {
        const response = await fetch('https://soundcloud-scraper.p.rapidapi.com/v1/track/metadata', {
            method: 'GET',
            headers: {
                'X-RapidAPI-Key': RAPIDAPI_KEY,
                'X-RapidAPI-Host': 'soundcloud-scraper.p.rapidapi.com'
            },
            params: { track: trackUrl }
        });
        const data = await response.json();
        return data.audio[0].url; // URL do áudio
    } catch (error) {
...
async function playTrack(element) {
    ...
    const cover = element.querySelector('.musica-img img').src;
...
// Adicionar evento de clique às músicas
document.querySelectorAll('.musica-img').forEach(img => {
